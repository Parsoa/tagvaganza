...
def find_closest_release(artist, release):
    pretty_print('searching for closest album to', green(release.title), white('with'), green(release.get_num_tracks()), white('tracks'))
    results = get_all_artist_releases(artist['id'])
    # 1. find all releases of this artist with a matching name
    choice = find_minimum_cost_match(results, release.title, 'title')
    if not choice:
        pretty_print(colorama.Fore.RED + 'couldn\'t find a release with matching title, closest match: ', magenta(choice['title']))
        return None
    ...
    choice = list(filter(lambda x: x['medium-track-count'] == release.get_num_tracks(), results))
    if choice:
        match = choice[0]
